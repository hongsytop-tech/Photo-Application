...
5. **골라서 치우기** — 목록에서 길게 눌러 여러 장을 고른 뒤 **휴지통으로**
   보냅니다. 30일 안에는 갤러리 앱에서 되살릴 수 있습니다. 상단의 겹친 네모
   버튼을 누르면 **같은 사진의 복사본**만 한 번에 골라 줍니다(그룹마다 한 장은
   남깁니다).

목록은 두 손가락으로 벌리고 오므려 칸 크기를 바꿉니다. 메모가 붙은 사진은
오른쪽 아래에 배지가 뜨고, 그 배지를 누르면 팝업으로 바로 읽고 고칠 수 있습니다.
