...
void main() {
  ...
  group('태그 분류', () {
    test('분류를 만들고 태그를 넣으면 그 분류로 잡힌다', () async {
      final people = await tags.ensureGroup('인물');
      final mom = await tags.ensure('엄마', groupId: people.id);

      expect(mom.groupId, people.id);
      final listed = await tags.listAll();
      expect(listed.single.groupId, people.id);
      expect((await tags.listGroups()).single.tagCount, 1);
    });

    test('분류를 지정하지 않은 태그는 미분류로 남는다', () async {
      await tags.ensureGroup('인물');
      await tags.ensure('바다');

      final listed = await tags.listAll();
      expect(listed.single.groupId, isEmpty);
      expect((await tags.listGroups()).single.tagCount, 0);
    });

    test('분류를 지워도 태그는 남고 미분류로 돌아간다', () async {
      final people = await tags.ensureGroup('인물');
      final mom = await tags.ensure('엄마', groupId: people.id);
      await addPhoto('a1', 'k1');
      await tags.attach('k1', mom.id);

      await tags.deleteGroup(people.id);

      expect(await tags.listGroups(), isEmpty);
      final listed = await tags.listAll();
      expect(listed.single.name, '엄마');
      expect(listed.single.groupId, isEmpty, reason: '미분류로 돌아가야 한다');
      expect(listed.single.photoCount, 1, reason: '사진과의 연결은 그대로다');
    });

    test('이미 있는 태그를 다시 만들어도 분류가 바뀌지 않는다', () async {
      final people = await tags.ensureGroup('인물');
      final places = await tags.ensureGroup('장소');
      await tags.ensure('엄마', groupId: people.id);

      // 사진에 태그를 다는 중에 같은 이름을 또 적은 상황.
      final again = await tags.ensure('엄마', groupId: places.id);

      expect(again.groupId, people.id);
      expect((await tags.listAll()).single.groupId, people.id);
    });

    test('같은 이름의 분류를 두 번 만들면 하나로 합쳐진다', () async {
      final first = await tags.ensureGroup('인물');
      final second = await tags.ensureGroup('인물');

      expect(second.id, first.id);
      expect((await tags.listGroups()).length, 1);
    });

    test('태그를 다른 분류로 옮길 수 있다', () async {
      final people = await tags.ensureGroup('인물');
      final places = await tags.ensureGroup('장소');
      final tag = await tags.ensure('제주', groupId: people.id);

      await tags.moveToGroup(tag.id, places.id);
      expect((await tags.listAll()).single.groupId, places.id);

      await tags.moveToGroup(tag.id, '');
      expect((await tags.listAll()).single.groupId, isEmpty);
    });

    test('분류는 만든 순서대로 나온다', () async {
      for (final name in ['인물', '장소', '행사']) {
        await tags.ensureGroup(name);
      }
      expect(
        (await tags.listGroups()).map((g) => g.name).toList(),
        ['인물', '장소', '행사'],
      );
    });
  });
