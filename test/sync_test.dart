...
void main() {
  ...
  test('서버 행에 빠진 칸이 있어도 내려받기가 멈추지 않는다', () async {
    // 예전 앱만 쓰던 계정의 행에는 나중에 생긴 칸이 없습니다. 그 null 을
    // 그대로 쓰면 NOT NULL 에 걸려 pull 전체가 뒤집힙니다.
    remote.tables['photo_folders'] = {
      'user-1|f1': {
        'user_id': 'user-1',
        'id': 'f1',
        'name': '여행',
        // sort_order 가 통째로 빠져 있음
        'updated_ms': 100,
        'deleted': 0,
      },
    };
    ...
    final folders = await b.folders.listAll();
    expect(folders.single.name, '여행');
  });
