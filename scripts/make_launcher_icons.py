...
# 카메라를 눈높이까지 올려 뜬 눈을 가립니다 — 한 눈은 뷰파인더 뒤, 다른 눈은
# 찡긋. 폭을 넓히는 대신 높이를 키워 볼터치 자리를 남겨 둡니다.
CAM = (30.5, 41.0, 9.8, 9.0)      # cx, cy, 반너비, 반높이
...
def draw(art):
    ...
    # 뜬 눈은 곧이어 카메라가 덮습니다. 그래도 그려 둡니다 — 카메라 위치를
    # 조금만 손대도 눈이 사라지거나 반쯤 나오게 되는데, 그리지 않으면 그 사실을
    # 눈으로 확인할 방법이 없습니다.
    art.flat(art.circ(widen(28.5, 6), 35.5, 3.0), INK, 80, amp=0.28)
    art.flat(art.circ(widen(27.5, 6), 34.5, 1.05), CREAM, 81, amp=0.2)
